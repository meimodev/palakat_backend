...
export class ApprovalRuleController {
  ...
  async create(@Body() createApprovalRule: Prisma.ApprovalRuleCreateInput) {
    ...
    let transformed = transformToSetFormat(createApprovalRule, ['positions'], 'connect');

    let cleaned = stripKeys(transformed, ['church', 'createdAt','updatedAt' ]);
  ...
  async update(
    ...
    @Body() updateApprovalRule: Prisma.ApprovalRuleUpdateInput,
      ...
      let transformed = transformToSetFormat(updateApprovalRule, ['positions']);
